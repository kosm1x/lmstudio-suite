...
- **`console.log` in a plugin surfaces in `lms log stream`** as `[Plugin(owner/name)] stdout: …`. When you can't see a plugin's internal decisions, add decision logs (trigger/matched/dir/write-outcome) — far faster than inferring from the server's model-side logs, which never show plugin file I/O.

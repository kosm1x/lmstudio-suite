...
- **A global-config default may arrive as `undefined` — guard every `.trim()` on it, especially outside try/catch.** The real "no file ever written" bug: `parseCompactTrigger` did `trigger.trim()` on `global.get("trigger")`, and LM Studio returned `undefined` for the trigger field (the global default `/compact` was never materialized because the user only ever set the Export directory). `undefined.trim()` threw — and the call sits **before** the export `try`, so the whole preprocessor threw, LM Studio **failed it open**, the model got the raw `/compact` (and improvised a summary), and the file code was never reached. Tells: no `[compact]` status line in the chat, and the failure is independent of the (correctly set) Export directory because execution dies before that line. Fixes: default the value at the read site (`(global.get(k) ?? "").trim() || DEFAULT`), make the pure parser null-safe, and never let a preprocessor throw uncaught. Also: a caught error returns a `[compact] …` string (so a status line shows); **no status line at all = an uncaught throw or passthrough _before_ the try** — that distinction localizes the bug instantly.
- **`console.log` in a plugin surfaces in `lms log stream`** as `[Plugin(owner/name)] stdout: …`. When you can't see a plugin's internal decisions, add decision logs (trigger/matched/dir/write-outcome) — far faster than inferring from the server's model-side logs, which never show plugin file I/O. (This is what finally cracked the compact bug — the log line `trigger="/compact" matched=false` proved the trigger was right but the _message_ was wrong.)
- **Prompt preprocessors CHAIN — a trigger-word plugin can't assume the user's text arrives verbatim.** Multiple enabled preprocessors run in sequence, each receiving the previous one's output. With `time`/`memory`/`rag` enabled _ahead_ of `compact`, they prepend content (current time, retrieved context) to the user message, so by the time `compact` ran, the text no longer _started_ with `/compact` → its `startsWith` trigger check returned no-match → passthrough → no file. This was the final layer under the compact bug (after the undefined-trigger throw). Fix: match the trigger as a **standalone line anywhere in the message** (scan `body` + each line; a line must still _start_ with the trigger so mid-sentence mentions and `/compacting` don't match), not just at the very start. General lesson: a slash-command-style trigger implemented as a preprocessor must be robust to upstream preprocessors mutating the message; test with prepended content.
...
- **Compact case study (revs 1→9) — the meta-lesson.** One feature, nine revisions, because the failures stacked and I diagnosed by inference instead of instrumentation. The arc: ship → budget overflowed (max-not-loaded context) → undefined-global-trigger threw before the try (fail-open) → preprocessor-chain prepended content so `startsWith` missed → then output quality/length. Each looked like the previous one "not deploying." What actually cracked it: **adding `console.log` decision lines** (trigger/matched/received/dir/wrote) so the next `/compact` printed its own truth in `lms log stream`. Distilled rules: (1) for a host-plugin you can't run, **instrument first, infer never** — one log line beats five guesses; (2) **read the component's own output before the host's** — a plugin's returned status string (or absence of one) localizes the bug faster than server logs; (3) a **trigger implemented as a preprocessor must survive upstream preprocessors mutating the message**; (4) **prefer a config field over probing** when the SDK can't reliably report a value (loaded context, working dir, trigger default); (5) **never `.trim()` an SDK-provided value outside a try** — defaults can arrive `undefined`.

...
- **Budget from the model's LOADED context, not its maximum — and measure with the real tokenizer.** The first map-reduce attempt still overflowed because it sized the budget from `getContextLength()`, which on some runtimes (e.g. Llama 4 in LM Studio) reports the model's **maximum** context (~100k), not the **loaded** window (e.g. 32k). With a huge budget the "fits in one call?" check passed and it single-shot the whole transcript again — a bug that survives every reinstall (it's code, not a stale install). Correct sources: **`getModelInfo().contextLength`** is the _loaded_ instance window; `getContextLength()` may be the max. Also don't estimate size by a chars/token ratio — measure with **`countTokens()`** so dense text (code/CJK) can't blow the budget, and **reserve the instruction wrapper's tokens** when sizing chunks (the chunk text + preamble is what's sent). `tokenSource()` returns `LLM | LLMGeneratorHandle`; only `LLM` has `getModelInfo`/`getContextLength`/`countTokens`, so feature-detect with `"x" in source` and fall back (loaded-context unknown → safe default; no tokenizer → chars/4 estimate).

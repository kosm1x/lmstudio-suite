// Bundled by lmstudio-suite (scripts/package-plugins.mjs) from packages/plugin-toolkit. Do not edit; regenerate instead.
...
// packages/plugin-toolkit/src/config.ts
...
// packages/core/src/web/http.ts
var DEFAULT_UA = "Mozilla/5.0 (compatible; LMStudioSuite/0.1; +https://lmstudio.ai)";
async function fetchWithTimeout(url, init = {}, ctl = {}) {
  const { timeoutMs = 15e3, signal } = ctl;
...
async function fetchJson(url, init = {}, ctl = {}) {
...
async function postForm(url, form, ctl = {}) {
  const res = await fetchWithTimeout(
    url,
    {
      method: "POST",
...
function isPrivateHost(hostname) {
  ...
  if (h.includes(":")) {
    ...
    const hex = /^(?:::ffff:|::)([0-9a-f]{1,4}):([0-9a-f]{1,4})$/i.exec(h) ?? /^64:ff9b::([0-9a-f]{1,4}):([0-9a-f]{1,4})$/i.exec(h);
    if (dotted) {
      h = dotted[1] ?? h;
    } else if (hex) {
...
// packages/core/src/web/html-to-markdown.ts
...
function decodeEntities(s) {
  return s.replace(/&#(\d+);/g, (_, d) => safeCodePoint(Number(d))).replace(
    /&#x([0-9a-fA-F]+);/g,
...
function isElement(n) {
  return n.nodeType === import_node_html_parser.NodeType.ELEMENT_NODE;
}
function isText(n) {
  return n.nodeType === import_node_html_parser.NodeType.TEXT_NODE;
}
function renderChildren(el) {
  return el.childNodes.map(render).join("");
}
function stripTags(raw) {
  return /<\/?[a-zA-Z]/.test(raw) ? (0, import_node_html_parser.parse)(raw).structuredText : raw;
}
    ...
    case "ol": {
      ...
      for (const child of node.childNodes) {
        if (isElement(child) && child.rawTagName?.toLowerCase() === "li") {
          ...
          const inner = renderChildren(child).trim().replace(/\s*\n\s*/g, " ");
...
`;
    ...
    case "tr": {
      ...
      for (const child of node.childNodes) {
        ...
        if (t === "td" || t === "th") {
          cells.push(
            renderChildren(child).trim().replace(/\s*\n\s*/g, " ")
          );
        }
      }
      return cells.length ? `
...
function htmlToMarkdown(html) {
  ...
  const main2 = root.querySelector("article") ?? root.querySelector("main") ?? root.querySelector("body") ?? root;
  return renderChildren(main2).replace(/[ \t]+\n/g, "\n").replace(/\n{3,}/g, "\n\n").replace(/[ \t]{2,}/g, " ").trim();
}
function extractTitle(html) {
  const root = (0, import_node_html_parser.parse)(html, { comment: false });
  const og = root.querySelectorAll("meta").find((m) => m.getAttribute("property") === "og:title")?.getAttribute("content");
  const title = og?.trim() || root.querySelector("title")?.text.trim() || root.querySelector("h1")?.text.trim() || "";
...
async function fetchPage(url, options = {}) {
  ...
  const looksHtml = contentType.includes("html") || /^\s*<(!doctype|html)/i.test(body);
  let title = current;
  let markdown;
  if (looksHtml) {
...
// packages/core/src/web/search.ts
...
function requireSearxng(config) {
...
async function searchSearxng(query, baseUrl, options) {
  const { maxResults = 5, ...ctl } = options;
  const u = new URL("/search", baseUrl);
  u.searchParams.set("q", query);
  u.searchParams.set("format", "json");
  const res = await fetchJson(u.toString(), {}, ctl);
...
function fileToNode(relPath, text, stat, options = {}) {
  ...
  const name = fmString(data, "name") ?? basename2(relPath, extname(relPath));
...
async function scanKbDir(root, options = {}) {
...
function planIncomingMoves(graph, options = {}) {
  ...
  for (const node of graph.nodes) {
    ...
    const to = `${folder}/${basename3(node.path)}`;
    if (graph.get(to)) {
...
function createWebTools(options) {
  ...
  const fetchMaxChars = options.fetchMaxChars ?? 8e3;
  const allowPrivateHosts = options.allowPrivateHosts ?? false;
  const webSearchTool = tool({
    name: "web_search",
    description: "Search the web and return a ranked list of results, each with a title, URL, and snippet. Use this to find current information, facts, or pages to open with fetch_url.",
    parameters: {
      query: z.string().describe("What to search for, as a natural-language query.")
    },
  ...
  const fetchUrlTool = tool({
    name: "fetch_url",
    description: "Fetch an http/https web page and return its main content as Markdown. Use after web_search to read a specific result, or to read a known URL directly.",
    parameters: {
...
var msg2 = (err) => err instanceof Error ? err.message : String(err);
    ...
    tool3({
      ...
      implementation: async ({ pattern, glob, path, ignore_case }, { status, warn }) => {
        ...
        try {
          ...
          return `Error: invalid regular expression: ${msg2(err)}`;
        }
...
var msg4 = (err) => err instanceof Error ? err.message : String(err);
    ...
    tool5({
      ...
      implementation: async ({ file, query }, { status, warn }) => {
        ...
        try {
          ...
          return `Error: cannot open SQLite database (is node:sqlite available?): ${msg4(err)}`;
        }
        try {
          const rows = [];
          let hasMore = false;
...
\u2026[showing the first ${SQLITE_MAX_ROWS} rows; more exist]` : body;
          ...
          warn(`query_sqlite failed: ${msg4(err)}`);
          return `Error: ${msg4(err)}`;
        } finally {
          try {
            db.close();
...
var msg5 = (err) => err instanceof Error ? err.message : String(err);
var RECALL_MAX = 10;
...
var msg6 = (err) => err instanceof Error ? err.message : String(err);
...
async function resolveRoot(ctl, configuredDir) {
  ...
  if (dir) {
    const expanded = dir === "~" || dir.startsWith("~/") ? join2(homedir(), dir.slice(1)) : dir;
    return resolve2(expanded);
  }
  try {
    return ctl.getWorkingDirectory();
  } catch {
    const fallback = join2(tmpdir(), "lmstudio-toolkit");
...
async function toolsProvider(ctl) {
  ...
  const root = await resolveRoot(ctl, global2.get("workingDir"));
    ...
    tools.push(
      ...createWebTools({
        search: {
          ...
          searxngUrl: global2.get("searxngUrl") || void 0
        },
        allowPrivateHosts
      })
    );
...
// packages/plugin-toolkit/src/index.ts
